...
class CPU{
  ...
  draw(){
    this.c.fillRect(800,this.y,25,100);
  }
}


//Animar CPU
function animarCPU(yp){
  if(yp <= cpu.y + 50){
  cpu.y -= cpu.dy;
  }
  if(yp >= cpu.y + 50){
...
if(nivell == "facil"){
  ...
  pil = new pilota(395,r(300)+50,7,7);
}else if(nivell == "expert"){
  pil = new pilota(395,r(300)+50,9,9);
}
...
function animació(){
  ...
  pil.context.clearRect(0,0,1100,1100);
  if (pil.x <= 31) {
    if(j1.y <= pil.y + 10 && j1.y + 105 >= pil.y + 5 || cpu.y <= pil.y + 10 && cpu.y + 105 >= pil.y + 5 ){
    ...
    if(cpu.y <= pil.y + 10 && cpu.y + 105 >= pil.y + 5){
      pil.dx = -pil.dx;
      pil.dx += 0.3;
      pil.dy += 0.3;
      cpu.p += 1;
    ...
    h1.innerHTML = punts + "p";
      ...
      var rec;
      if(nivell == "facil"){
        rec = window.localStorage.getItem("recordfcpu");
      }else if(nivell == "normal"){
        rec = window.localStorage.getItem("recordncpu");
      }else if(nivell == "dificil"){
        rec = window.localStorage.getItem("recorddcpu");
      }else if(nivell == "expert"){
        rec = window.localStorage.getItem("recordecpu");
      }
      if(rec == null){
        rec = 0;
      }
  ...
  j1.context.clearRect(0,0,1000,1000);
  pil.draw();
